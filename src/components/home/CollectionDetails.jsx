...
const fieldClass = 'mt-2 w-full rounded-xl border border-[#A9D5EF] bg-white px-4 py-3 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-[#38A9E0] focus:ring-3 focus:ring-[#38A9E0]/15'
...
export default function CollectionDetails({ onSubmit, saturdayCollection, setSaturdayCollection, collectionDate, setCollectionDate, paymentOption, setPaymentOption, notice, setNotice, accessConfirmed, setAccessConfirmed, restrictedAccess, setRestrictedAccess }) {
  ...
  return (
    <form id='collection-details' onSubmit={onSubmit} className='mt-10 overflow-hidden rounded-2xl border border-[#A9D5EF] bg-[#F5FBFF] shadow-sm'>
      <div className='border-b border-[#B8DDF3] bg-[#E4F4FD] px-4 py-5 sm:px-6'>
        <p className='text-xs font-extrabold uppercase tracking-[0.18em] text-[#1687BE]'>One last step</p>
        <h3 className='mt-1 text-xl font-black text-slate-800 sm:text-2xl'>Collection details</h3>
        <p className='mt-1 text-sm text-slate-600'>Tell us how to plan your collection. Required fields are marked with an asterisk.</p>
      </div>
      <div className='grid gap-6 p-4 sm:p-6 lg:grid-cols-2 lg:gap-8'>
        ...
        <fieldset className={`rounded-xl border bg-white p-4 transition ${accessConfirmed ? 'border-[#38A9E0] ring-2 ring-[#38A9E0]/10' : 'border-[#C9E5F5]'}`}>
        ...
        {accessConfirmed && (
          <fieldset className='rounded-xl border border-[#A9D5EF] bg-[#EAF7FE] p-4'>
            ...
            <div className='mt-3 flex gap-5'>
              {['yes', 'no'].map((answer) => <label key={answer} className='flex cursor-pointer items-center gap-2 text-sm font-bold capitalize text-slate-800'><input type='radio' name='restrictedAccess' value={answer} checked={restrictedAccess === answer} onChange={() => setRestrictedAccess(answer)} required className='size-5 accent-[#249BD3]' />{answer}</label>)}
            </div>
        ...
        <fieldset className={`rounded-xl border border-[#C9E5F5] bg-white p-4 ${accessConfirmed ? 'lg:col-span-2' : ''}`}>
          <legend className='px-1 text-base font-extrabold text-slate-800'>Collection date *</legend>
          <div className='grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(15rem,1fr)] sm:items-end'>
            ...
            <label className='flex min-h-11 cursor-pointer items-center justify-between gap-3 rounded-lg bg-[#EAF7FE] p-3'>
        ...
        <fieldset className='lg:col-span-2'>
          ...
          <div className='mt-3 grid gap-3 sm:grid-cols-2'>
            {[{ id: 'now', title: 'Pay now', text: 'Pay the full amount securely online.' }, { id: 'arrival', title: 'Pay on arrival', text: 'Pay the £25 callout fee now and the balance after collection.' }].map((option) => (
              <label key={option.id} className={`cursor-pointer rounded-xl border p-4 transition ${paymentOption === option.id ? 'border-[#38A9E0] bg-[#E4F4FD] ring-2 ring-[#38A9E0]/15' : 'border-[#C9E5F5] bg-white hover:border-[#38A9E0]'}`}>
        ...
        <fieldset className='lg:col-span-2'>
          ...
          <div className='mt-3 flex flex-wrap gap-3'>
            {['30', '60'].map((minutes) => <label key={minutes} className={`flex cursor-pointer items-center gap-2 rounded-full border px-4 py-2.5 text-sm font-bold transition ${notice === minutes ? 'border-[#38A9E0] bg-[#E4F4FD] text-[#1479A9]' : 'border-[#C9E5F5] bg-white text-slate-600'}`}><input type='radio' name='notice' value={minutes} checked={notice === minutes} onChange={() => setNotice(minutes)} className='size-4 accent-[#249BD3]' />{minutes} minutes</label>)}
          </div>
        </fieldset>

        <label className='lg:col-span-2'><span className='text-sm font-bold text-slate-800'>Collection notes</span><textarea name='collectionNotes' rows={3} placeholder='For example: gate code, floor number, parking or access details' className={fieldClass} /></label>
      </div>
    </form>
  )
}