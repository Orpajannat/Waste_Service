    ...
    <div className='relative z-20 mx-auto grid w-full max-w-7xl grid-cols-1 gap-5 px-4 py-8 sm:grid-cols-2 sm:px-6 sm:py-10 lg:grid-cols-3 lg:gap-7 lg:px-8 lg:py-14'>
      {services.map((service) => (
        <article
          key={service.title}
          tabIndex={0}
          className='group relative min-w-0 overflow-hidden rounded-lg bg-[#11224D] p-4 shadow-xl transition duration-300 hover:-translate-y-1 hover:shadow-2xl focus-visible:-translate-y-1 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white sm:p-5'
        >
          ...
          <div className='absolute inset-x-0 bottom-0 flex h-full translate-y-[calc(100%_-_5rem)] flex-col items-center bg-[#11224D] px-5 text-center text-white transition-transform duration-500 ease-out group-hover:translate-y-0 group-focus:translate-y-0 sm:px-7'>
